...
Three question types, one answer type. The three mirror TypeSafe's System One
primitives (``docs.typesafe.ai/api``) because that is the only surface with a
wire format to honour; the LLM implementation answers the SAME triple so a
shadow row from either implementation is comparable field-for-field. A fourth
type would have to be added to both implementations AND to every point file's
``agree`` definition, so the set is deliberately closed here.
...
* ``value`` -- the chosen option (``Choice``), the level (``Score``, in the
  question's own ``lo..hi`` units, possibly fractional), or the yes-probability
  (``Noul``).
...
@dataclass
class Score:
    """Rate the state on the inclusive integer ladder ``lo..hi``.

    The bounds are the units the ANSWER comes back in, so a point file reads
    ``value`` directly against its own scale instead of rescaling. ``hi`` must
    exceed ``lo`` -- a one-rung ladder has nothing to choose between -- and the
    answer may land between rungs, because it is a probability-weighted value
    across them rather than a pick.
    """
...
#: One question of any of the three types. A union rather than a base class:
#: the three carry disjoint fields and no shared behaviour, so a base class
#: would only be a place for a future ``ask``-time branch to hide.
