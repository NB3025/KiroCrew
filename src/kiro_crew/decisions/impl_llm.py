...
make the two look comparable when they are not. ``p`` for a Choice/Score comes
back as ``null`` -> 0.0, which the report reads as "not reported"; for a Noul the
...
def build_prompt(state: dict | str, questions: list[Question]) -> str:
    ...
    for q in questions:
        if isinstance(q, Choice):
            ...
            lines.append(f"- {q.id}: {q.prompt} Answer with an integer from {q.lo} to {q.hi}.")
            shape.append(f'  "{q.id}": {{"value": <{q.lo}..{q.hi}>, "p": <0..1>}}')
...
def _answer_from_entry(q: Question, entry: dict) -> Answer:
    ...
    if isinstance(q, Score):
        ...
        # Clamped to the ladder rather than refused: unlike a Choice option, a
        # score is ordered, so an out-of-range answer still carries a direction
        # and the nearest rung is the model's evident intent.
        return Answer(
            id=q.id, value=min(float(q.hi), max(float(q.lo), value)), p=p, confidence=None
        )
    ...
    if isinstance(q, Noul):
        ...
        return Answer(id=q.id, value=value, p=p or value, confidence=None)

    raise ValueError(f"unsupported question type {type(q).__name__}")


def _as_float(raw: Any) -> float:
    """*raw* as a float, or 0.0 (which the report reads as "not reported")."""
    try:
        return float(raw)
