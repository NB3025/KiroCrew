...
def has_credential(state: dict | str) -> bool:
    """Whether *state* contains anything that reads as a credential.

    TWO scanners, and either one is enough to refuse. ``_CREDENTIAL_RE`` is the
    pattern list this module compiles; ``redact_credentials`` is the repo's
    canonical output scanner, which ``skills.py`` and ``tips.py`` already run on
    text leaving the machine. Neither one contains the other:

    * only ``redact_credentials`` matches a BARE 40-char secret (an entropy floor
      plus structural gates), the ``aws_secret_access_key=`` / ``SessionToken=``
      key-value forms, a PEM private-key block, and base64-encoded credentials;
    * only ``_CREDENTIAL_RE`` matches the generic ``sk-[A-Za-z0-9]{20,}`` vendor
      spelling that ``credential_patterns`` exports.

    A gate that refused only the narrower set would send a pasted AWS secret to a
    third party while the same string is redacted out of this machine's own logs.

    The canonical scan runs SECOND and only when the cheap one found nothing: it
    is the more expensive of the two, and it only ever runs on a point that is
    already armed -- gate 4 sits below gates 1-3, so the default configuration
    reaches neither scanner.

    Its warnings, not its cleaned text, are the signal: the state is refused
    whole rather than sent redacted, because a judge's answer over redacted state
    would be logged as if it had judged the real thing. A failure inside the
    scanner reads as "credential present": gate 4's job is to refuse what it
    cannot clear.
    """
