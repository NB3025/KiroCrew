...
|---|---|---|
| `skills.select` | Which of these skills are relevant to this message? | The trigger matcher's overlap score |
| `skills.dedupe` | Is this extracted skill new, a duplicate, or an update? | The verdict from the existing model call |
| `cron.novelty` | Does this cron result say anything new against the last one? | The byte-equality check on the result hash |
...
`impl` chooses who answers — a small dedicated judge, or a language model answering the same question, which is the control you compare it against. `bucket` is a percentage of sessions, so `bucket: 10` samples a tenth of them and leaves the rest untouched. Settings carries a card for the feature-wide switch; the per-point arms are config values.
...
What is in the question, per point: for `skills.select`, the message you just typed and the skill descriptions; for `skills.dedupe`, the candidate skill text; for `cron.novelty`, that job's last result. Memory contents, file diffs, and credentials are not in any of them. Requests are scrubbed for credential patterns first, and a hit cancels the call outright rather than sending a redacted version — the log records `scrubbed` so you can see it happened.
