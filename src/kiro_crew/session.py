...
#
# codex is what makes the two terms differ: it runs on the shared runtime and its
# ``session/cancel`` leaves the session addressable with its context resident, so
# it is a member of the first set and absent from the second. A harness reaches
# this path by demonstrating BOTH, and Crew closing the eviction gap — codex
# advertises ``session/close`` and ``session/delete`` — is what would add it,
