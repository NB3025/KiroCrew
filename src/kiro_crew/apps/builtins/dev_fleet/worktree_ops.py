...
async def _prune_candidates() -> dict:
    ...
    # up to two networked gh calls, so a plain serial loop scales with fleet
    # size: a 111-worktree fleet floors at ~57s even at one gh call each,
    # double the gateway app proxy's 30s _PROXY_TIMEOUT, so the preview returns
    # 504 and the button never renders. Run the verdicts concurrently under the
    # same _PRUNE_CONCURRENCY bound the parallel prune workers use -- this path
    # is read-only git (rev-parse, status, rev-list/cherry, merge-base) so it
    # never touches _GIT_MUTATION_LOCK, which only the destructive removal path
    # holds.
