...
# The command-line fragment that identifies a device login started by
# :func:`_device_login_command`. It is deliberately a SUBSTRING of what that
# function launches: `$KIRO` always resolves to a path whose basename is
# `kiro-cli` (see `_KIRO_BIN_RESOLVE`), and the flags follow, so the launched
# command line contains this fragment whether or not `stdbuf` wraps it and
# whatever identity flags are appended. `test_cloud_signin_recovery.py` asserts
# that coupling against the real builder, so a rename on either side fails a
# test rather than silently matching nothing on the box.
_LOGIN_PROCESS_PATTERN = "kiro-cli login --use-device-flow"
