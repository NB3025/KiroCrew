    ...
    def reap_orphans(self) -> List[str]:
        ...
        for job in self.list():
            ...
            if job.step(STEP_CONNECT).state == STEP_DONE:
                ...
                # when the approval landed, and "Get a new code" replaces the login
...
def run_signin_retry(
    ...
    *,
    ...
    This is the dashboard's "Sign in" / "Get a new sign-in code" action. The
    ...
    try:
        handle = _begin_signin_with_target(engine, job)
        started = handle
        # The box has replaced the login: the old poller is dead and the old code
        # cannot authenticate anything. Only now is it safe to drop it.
        job.signin = None
        ...
        try:
            if signin_error:
                ...
                s.state = STEP_SKIPPED
                s.detail = "Sign in from the box below once it opens."
        finally:
            try:
                handle.close()
            except Exception:  # pragma: no cover - best effort
        ...
        if started is not None:
            # A new login was started: the box already replaced the old one, so
            # the new poller is the only one alive and must be stopped. Dropping
            # the code locally is not cancelling -- it is in a browser.
