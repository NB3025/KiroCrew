    ...
    try:
        ...
        if _retry_eligible:
            ...
            if _set_model_fn is None:
                ...
                # under the SAME lock every other writer of those fields takes
                # (explicit pick + bulk pick in chat_handlers, throttle swap and
                # restore probe above). This reactive path was the one writer
                # that skipped it: a forced bulk pick (skip_running=false) is a
                # documented concurrent API call that can land in the set_model
                # await below, commit its own model + tear the session down,
                # after which our unlocked state writes would record a stale
                # fallback record and diverge slot.model from the live session.
