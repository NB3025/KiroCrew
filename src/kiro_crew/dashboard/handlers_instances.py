...
from kiro_crew.constants import is_owner_controlled_session_key
...
async def _authorize_crew_read(request: web.Request, operation: str) -> "web.Response | None":
    ...
    **Two claim checks and one allowlist**, because a denylist of identity classes
    cannot secure this: a channel-born session carries no app claim, and the legacy
    app-platform ``channel:`` prefix is absent from
    :data:`CHANNEL_SESSION_NAMESPACES` by design, so each enumerated refusal leaves
    the next unenumerated one admitted:
    ...
    2. *Caller namespace.* A PRESENT ``X-Session-Key`` must sit in
       :data:`OWNER_CONTROLLED_SESSION_NAMESPACES`. This is the inverted check: a
       ...
       indistinguishable from the owner under an app-only test. Refusing by
       allowlist means an unrecognised namespace — the legacy ``channel:`` shape, or
       a transport added later — is refused rather than admitted, so an unforeseen
    ...
    The namespace test reads ``X-Session-Key``, which is server-set and attested:
    ...
    decision must come from outside agent-writable storage.
    ...
    rather than a subject it can adjudicate.
    """
    ...
    if caller_key and not is_owner_controlled_session_key(caller_key):
        _audit(operation, "denied", error="untrusted caller namespace rejected")
        return web.json_response({"error": "forbidden", "code": "owner_only"}, status=403)
