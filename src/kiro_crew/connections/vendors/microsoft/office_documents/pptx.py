...
def _slide_parts(path: str, names: list[str]) -> list[tuple[int, str]]:
    ...
    means by "slide N". Falls back to filename-number order only when the id
    list or its rels cannot be resolved (a malformed package).
    """
    name_set = set(names)
...
def _slide_order_via_sldidlst(path: str, names: set[str]) -> list[str] | None:
    """Resolve slide parts in ``sldIdLst`` order, or None if unresolvable.

    Returns the ordered slide part names, or ``None`` when the presentation
    part, its rels, or the id list is missing/malformed so the caller can fall
    ...
    for sld_id in sld_id_lst:
        ...
        if part is None:
            return None  # dangling id -> fall back rather than skip a slide
        ordered.append(part)
...
def replace_slide_text(
    ...
    *edits* maps a one-based PRESENTATION position to its new text (the same
    ...
    # Pin the source signature for in-place concurrent-writer detection (F2).
