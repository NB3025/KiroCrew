...
def _rewrite_document_xml(raw: bytes, edits: dict[int, str]) -> bytes:
    ...
    *edits* maps a zero-based body-paragraph index to its new plain text. The
    ...
    for idx, new_text in edits.items():
        ...
        if text_runs:
            first = text_runs[0]
            # Collapse the first text run to its <w:rPr> (run properties —
            # bold/italic/color/font) plus one <w:t> holding the new text.
            # Keep <w:rPr> so the edited text keeps its character formatting;
            # remove only the text-content children (<w:t>/<w:br>/<w:tab>).
            rPr = first.find(f"{C.W}rPr")
            for sub in list(first):
                if sub.tag != f"{C.W}rPr":
                    first.remove(sub)
            ...
            if rPr is not None and list(first)[0] is not rPr:
                first.remove(rPr)
                first.insert(0, rPr)
            # Drop the OTHER text-bearing runs, wherever they sit in the subtree.
...
def replace_paragraph_text(
    ...
    *edits* maps zero-based body-paragraph index to new text. ``src_path`` and
    ...
    # Pin the source's signature before reading the part we will edit, so an
    # in-place edit detects a concurrent writer between read and republish
    # (F2). Only meaningful for in-place; a distinct dst has nothing to race.
    sig = container.source_signature(src_path) if src_path == dst_path else None
    if not edits:
