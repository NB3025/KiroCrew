...
def darwin_pgroup_members(pgid: int) -> list[DarwinKinfoProc] | None:
    ...
    with slack for processes that join between the two calls; an answer that
    still overflows is refused (None) rather than returned truncated.
    """
    if pgid <= 0:
        return None
    libc = _darwin_sysctl_handle()
    ...
    try:
        ...
        return None
    capacity = size.value + _DARWIN_KINFO_PGRP_SLACK * _DARWIN_KINFO_PROC_SIZE
    raw = _darwin_kinfo_query(_DARWIN_KERN_PROC_PGRP, pgid, capacity)
    if raw is None:
        return None
