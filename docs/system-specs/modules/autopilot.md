...
7. **Wait for the stage's sub-agents.** Polls
   ...
   The cap is half the configured stage timeout, up to 450 rounds (15 minutes);
   a disabled stage timeout uses the 15-minute cap. The effective key is
   required for a channel-born slot, whose children belong to the linked
   `slack:*` (or other channel) session rather than `dashboard:<slot>`. This is
   **fail-closed**: a missing manager, or `running_agents_for` returning `None`
   ...
   running agents yet. An authentication refusal before the stage prompt is
   consumed marks that stage for same-stage retry after login; a refusal while
   delivering a completion requeues the system input. Each boundary requires
...
Each stage retains every immutable parent-session key its turns capture, so a
mid-stage slot link cannot move settlement away from existing children or reports.
A completion turn that fails before consumption requeues its exact system entry,
and the controller performs its final queue arbitration after its last await.
