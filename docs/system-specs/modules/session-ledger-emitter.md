...
emit would then cite the replacement's ledger and corrupt this child's immutable
...
over each session ledger's `session/opened`, keyed by SLOT (the creator's `sid`
