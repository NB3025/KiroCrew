...
is the only member. Codex is deliberately out, and it is out on CREW's limit
rather than the harness's: the adapter would hold a shared session (its teardown
leaves one addressable), but the shared-subagent path persists a provider label
`SessionMap` reads as kiro-cli, so `spawn_continue` on a codex subagent answers
`conversation_gone`. KAS sits in the same position for its own reason — on the
runtime, out of this set until the missing piece lands — and codex follows that
precedent rather than re-arguing it. The invariants governing what an
