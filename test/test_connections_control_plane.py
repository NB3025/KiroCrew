...
def test_DEFECT2b_a_malformed_record_inside_valid_json_fails_closed(tmp_path) -> None:
    ...
    with pytest.raises(BindingStoreCorruptError):
        store.all_bindings()
    # And EVERY mutation refuses rather than republishing a filtered (record-losing)
    # map -- this is the defence for the "binding permanently deleted" outcome, so
    # each of insert / rotate / revoke is asserted, not reasoned about.
    with pytest.raises(BindingStoreCorruptError):
        _insert(store, _mk_binding(subject="carol", tenant="acme"))
    with pytest.raises(BindingStoreCorruptError):
        store.rotate(good["binding_id"], observed_generation=good["generation"])
    with pytest.raises(BindingStoreCorruptError):
        store.revoke(good["binding_id"])
...
def test_DEFECT2b_a_zero_byte_store_is_rejected_as_tamper_not_empty(tmp_path) -> None:
    # F3: a zero-length (or whitespace-only) file is NOT a genuinely empty store --
    # atomic_write never produces one for a populated store -- so it must be
    # treated as tampering and fail closed, not read as {} (which would let the
    # next write republish an empty store and erase every binding).
    store = _fresh_store(tmp_path)
    _insert(store, _mk_binding(subject="alice", tenant="acme"))
    store.path.write_text("", encoding="utf-8")  # zero bytes
    with pytest.raises(BindingStoreCorruptError):
        store.all_bindings()
    # whitespace-only is the same class.
...
def test_JUDGEMENT_fencing_reads_the_live_store_after_revoke(tmp_path) -> None:
    ...
    # Do NOT reveal or log the secret value OR anything a taint analysis treats as
    # secret-derived (py/clear-text-logging-sensitive-data). The evidence is the
    # NON-secret envelope, asserted directly -- no print sink at all.
