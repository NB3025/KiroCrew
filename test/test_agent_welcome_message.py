...
class TestSpecWelcomeMessage:
    ...
    def test_over_the_ceiling_is_truncated_with_an_ellipsis(self) -> None:
        ...
        assert len(out) == WELCOME_MESSAGE_MAX_CHARS + 1
        assert out.endswith("\u2026")
        assert out[:-1] == "y" * WELCOME_MESSAGE_MAX_CHARS
...
class TestAgentWelcomeMessageResolution:
    ...
    def test_declared_name_wins_over_a_matching_filename(self, fake_home) -> None:
        """A package manager namespaces specs on disk while the declared ``name``
        stays bare, so a stem match must never outrank a declared one — otherwise
        the hint shown belongs to a different agent than the one answering."""
        d = _agents_dir(fake_home)
        (d / "helper.json").write_text(
            json.dumps({"name": "someone-else", "welcomeMessage": "wrong agent"})
        )
        (d / "pkg-helper.json").write_text(json.dumps({"name": "helper", "welcomeMessage": _HINT}))
        clear_list_agents_cache()
        assert agent_welcome_message("helper", agents_dir=d) == _HINT

    def test_filename_match_is_used_when_nothing_declares_the_name(self, fake_home) -> None:
        d = _agents_dir(fake_home)
        (d / "helper.json").write_text(json.dumps({"welcomeMessage": _HINT}))
        clear_list_agents_cache()
        assert agent_welcome_message("helper", agents_dir=d) == _HINT
...
class TestSurfaceAgentWelcome:
    ...
    @pytest.mark.asyncio
    async def test_hint_lands_as_a_notice_row(self, monkeypatch, tmp_path) -> None:
        ...
        rows = [m for m in slot.messages if m.get("content") == _HINT]
        assert len(rows) == 1
...
class TestWelcomeMessageInARunnerTurn:
    ...
    @pytest.mark.asyncio
    async def test_the_default_crew_renders_nothing(self, monkeypatch, tmp_path) -> None:
        """An empty ``slot.agent`` is the default crew, which has no spec."""
        state, slot, client, _run_chat = self._harness(
            monkeypatch, tmp_path, _HINT, is_new=True, agent=""
        )
        self._text_stream(client)
        try:
            await _run_chat(state, slot, "hello")
