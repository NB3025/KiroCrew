...
class TestReportReadsWhatTheGateWrote:
    ...
    def test_iter_log_reads_the_same_rows(self, armed):
        armed(cron_novelty.POINT, _FixedOracle({"has_new_info": 0.9}))
        asyncio.run(cron_novelty.shadow_cron_novelty("j", "t", "a", "b", session_key="s-13"))
        rows = list(log_mod.iter_log())
        assert len(rows) == 1
        assert rows[0]["point"] == "cron.novelty"
