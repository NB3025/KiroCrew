...
def test_each_mcp_seam_is_spliced_only_for_its_own_harness() -> None:
    ...
    for fn in (
        ...
        # Non-vacuous by construction: the property is asserted for every harness
        # whose hook is spliced here, and a path that splices none is itself the
        # regression -- the array would then come from nowhere.
        assert hooks, f"{fn.__name__}: no per-harness MCP seam is spliced at all"
        for h in hooks:
            # opencode is the member whose array a stray element costs entirely: a
            # malformed entry fails the WHOLE session/new with -32602 there, so an
            # ungated splice of ANOTHER harness's hook into an opencode session is the
            # worst-consequence version of this defect, not the mildest.
            assert f"if self._is_{h}" in source, f"{fn.__name__}: {h} seam spliced ungated"
