...
SCORE = Score(id="relevance", prompt="How relevant?", lo=1, hi=5)
...
class TestPrompt:
    def test_every_question_appears_once(self):
        prompt = build_prompt("some state", [CHOICE, SCORE, NOUL])
    ...
    def test_score_bounds_are_stated(self):
        prompt = build_prompt("s", [SCORE])
            ...
            json.dumps(
                {
                    ...
                    "relevance": {"value": 4, "p": 0.6},
                    "novel": {"value": 0.2, "p": 0.2},
                }
            )
        )
        asyncio.run(LlmOracle().ask("s", [CHOICE, SCORE, NOUL]))
        assert len(calls) == 1
...
class TestParse:
    ...
    def test_a_score_comes_back_on_its_own_ladder(self):
        answers = parse_reply(json.dumps({"relevance": {"value": 4, "p": 0.6}}), [SCORE])
        assert answers["relevance"].value == pytest.approx(4.0)

    def test_a_score_outside_the_ladder_is_clamped(self):
        """Ordered, so an out-of-range answer still carries a direction."""
        assert parse_reply(json.dumps({"relevance": {"value": 99}}), [SCORE])[
            "relevance"
        ].value == pytest.approx(5.0)
        assert parse_reply(json.dumps({"relevance": {"value": -3}}), [SCORE])[
            "relevance"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Score/Noul value is not numeric.
            ('{"relevance": {"value": "very"}}', [SCORE]),
