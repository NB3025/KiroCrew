 ...
 *   version `rcN` — not only `-insider.N` but `-rc.N`, `-beta.N`, whatever
 *   label the tag carried — so the desktop spelling is `<letters><N>` with any
 *   letters, and `rcN` is what the wheel says back.
 ...
 * verbatim, so it compares equal only to another empty tail.
 */
