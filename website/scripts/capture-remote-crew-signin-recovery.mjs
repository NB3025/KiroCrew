...
for (const theme of ['dark', 'light']) {
  ...
  if (!/Approve the code for Kiro Crew Cloud \(kc-5e10bb\) with your company SSO account/i
...
for (const theme of ['dark', 'light']) {
  ...
  if (await prompt.getByRole('button', { name: /new sign-in code/i }).count()) {
  ...
  if (await page.getByText(/Launching…/).count()) {
    fail(`${theme}: "Launching…" must not sit above a card asking the reader to approve a code`)
  }
...
for (const theme of ['dark', 'light']) {
  ...
  const replace = prompt.getByRole('button', { name: /Get a new sign-in code/i })
  ...
  // recheck, and that asking for a new one retires it.
  ...
  if (!/Getting a new code retires it/i.test(hintText)) {
...
for (const theme of ['dark', 'light']) {
  ...
  if (!/Getting your sign-in code…/i.test(text)) {
    fail(`${theme}: the starting state must show the code spinner: ${JSON.stringify(text)}`)
  }
  // Nothing to approve and nothing to replace yet: offering either would be a
  // button that cannot work.
  if (await prompt.getByRole('button', { name: /new sign-in code|Show the sign-in code/i }).count()) {
